...
function UnderFileNamer:init(options)
    ...
    self.name_text    = options.name_text    or mod["nameText"]    or "Name the fallen human."
    self.confirm_text = options.confirm_text or mod["confirmText"] or "Is this name correct?"

    if options.goner_style then
...
function UnderFileNamer:checkTransition(old, new)
...
function UnderFileNamer:setState(state)
...
function UnderFileNamer:update()
